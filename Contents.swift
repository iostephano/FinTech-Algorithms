...
// Ejemplo 3.4
// Nota: se construye el valor con Decimal(string:) porque un literal como 2.345
//       pasa por conversión binaria y puede no ser exacto (p. ej. 2.345000000000000512),
//       lo que alteraría el resultado del redondeo half-to-even.
let r1 = bankersRound(Decimal(string: "2.345")!, scale: 2) // → 2.34
let r2 = bankersRound(Decimal(string: "2.355")!, scale: 2) // → 2.36
...
// Qué hace: genera un hash seguro, no reversible.
// Nota: SHA-256 por sí solo NO es apto para almacenar contraseñas en producción
//       (falta sal y una KDF lenta como Argon2, bcrypt o PBKDF2). Aquí solo
//       ilustra hashing genérico de propósito general.
// Complejidad: O(n)
...
func averageByCategory(_ txs: [(String, Decimal)]) -> [String: Decimal] {
    ...
    for (cat, total) in sums {
        result[cat] = total / Decimal(counts[cat] ?? 1)
    }
    return result
}
